...
*/
	...
	private ArrayList<BathroomStruct> bathrooms = new ArrayList<BathroomStruct>();
	...
	public  void bathroom(int id, int author, String gender, int shower, int sink, int papertowels, int active, String location, double x, double y, String building, int floor, String hours, int rating, int raters, int date)
	{
	...
	public  void bathroom(int alPos)
	{

		current = bathrooms.get(alPos);
	...
	public ArrayList getRestrooms(long lng, long lat)
	{
		...
		database = new UserAdaptor(ApplicationProvider.getApplicationContext());
		bathrooms = database.getBathroomArray();

		for(int i = 0; i < bathrooms.size; i++)
		{

			if(!(bathrooms.get(i).XCORD == lat && bathrooms.get(i).YCORD == lng))
			{
